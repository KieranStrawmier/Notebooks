...
tags: []
---
The New York Mets had a legitimate chance to make the 2019 playoffs. They had 10 players with an OPS+ higher than 100, the NL CY Young Award winner in Jacob deGrom and all six starters had an ERA+ of slightly below or above 100. So why did they not make the playoffs? 
...
ggplot() +
  ...
  geom_step(data = mil, aes(x= Gm, y=Wins, group= Tm),color="blue") +
    ...
    annotate("text", x=141, y=75, label="Brewers") +
    annotate("text", x=134, y=85, label="The Meltdown") +
    annotate("text", x=150, y=89.6, label="The Last Wild Card Spot") +
...
ggplot() +
    ...
    geom_vline(xintercept = 4.298944) +
      annotate("text", x=1.8, y=.7, label="Good R allowed/Good H-W allowed", color="Purple", size=3.5) +
      annotate("text", x=1.8, y=2.35, label="Good R allowed/Bad H-W allowed", color="Purple", size=3.5) +
      annotate("text", x=8, y=2.35, label="Bad R allowed/Bad H-W allowed", color="Purple", size=3.5) +
      annotate("text", x=8, y=.7, label="Bad R allowed/Good H-W allowed", color="Purple", size=3.5) +
      annotate("text", x=9,y=1.33,label="League Average") +
  ...
  labs(
    ...
    subtitle = "The Mets 9th inning pitchers gave up a lot of runs, walks and hits.",
    caption = "Source: baseball Reference | By: Kieran Strawmier"
  ) +
 theme(
    plot.title = element_text(size = 20, face = "bold"),
...
```
The best reliever in the National League was Josh Hader. The three pitchers in the 9th inning were in a different area code. While they were all had around average ERAs or WHIPs, they still were relatively below average. logic says you put your best guy in the game when you need the win. 
...
Easier said than done when considering the poor bullpen preformance throughout the year.
...
ggplot() +
  ...
  annotate("text", x=0.24, y=.452, label="League Average") +
    annotate("text", x=.369, y=.68, label="Good on base/Good power", color="Purple", size=3.5) +
    annotate("text", x=.369, y=.2, label="Good on base/Bad power", color="Purple", size=3.5) +
    annotate("text", x=.25, y=.2, label="Bad on base/Bad power", color="Purple", size=3.5) +
    annotate("text", x=.25, y=.68, label="Bad on base/Good power", color="Purple", size=3.5) +
  ...
  labs(
    ...
    subtitle = "The Nationals 9th inning batting order were some of the best at hitting for power and getting on.",
    caption = "Source: baseball Reference | By: Kieran Strawmier") +
 theme(
    plot.title = element_text(size = 20, face = "bold"),
    axis.title = element_text(size = 9), 
...
```
So, the eight players that walked up to the plate this inning were pretty scary. Three of them were near the best all-time baseball player, Mike Trout. Having slugging percentages in the upper .500s and on base percentages in the .400s.

So at any given moment the Nationals had the abilty to put up a handful of runs. 

The Met's win expectancy was at or above 50% for 70% of the game. So how did they go from 100% win expectancy to 0% in just eight plays?
...
wwe <- allplays %>% 
  group_by(
    Date, Away, AwayScore, Home, HomeScore, Inn
    ) %>% 
  summarize(
    peak = max(wWE), valley = min(wWE)
    ) %>% 
  mutate(
    difference = peak - valley)%>% arrange(desc(difference)
    )
```
...
The Mets started the inning with a 5-4 lead. Brandon Nimmo took the first pitch and sent it over the wall. The Mets scored four more runs and presumably took to soul out of the Nationals going into the bottom of the ninth.
...
Luis Avilan replaced Sewald after Rendon singled. Avilan was an average as a pitcher in 2019 with a slightly below average ERA.

After four pitches and Soto's single, Edwin Diaz replaced Avilan. Diaz had elite numbers when it came to everything except, HardHit%, Barrel% and average exit velocity.So when contact was made, which was rare with a whiff% in the 98% percentile, it was hit hard and far.
